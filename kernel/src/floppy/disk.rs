//! Handles raw data transfers to and from the floppy's disk.

use crate::{
    floppy::{
        CYLINDERS, DRIVE_ONE, FloppyCommand, FloppyError, FloppyPort, HEADS, SECTOR_SIZE, SECTORS,
        ST0_ERR_OR_RESET, TIMEOUT, fifo, motor,
    },
    startup, time,
};
...
pub fn write(ptr: u64, buf: &[u8]) -> Result<(), FloppyError> {
    ...
    'write: for _ in 0..DISK_RETRIES {
        ...
        /*/ FIXME: reading status fails and the next command being sent fails, causing a fdc reset
...
unsafe fn send_read_write(read: bool, ptr: u16, sects: u16) -> Result<(), FloppyError> {
    ...
    for _ in 0..SEEK_RETRIES {
        ...
        unsafe {
            fifo::seek(None)?; // FIXME: first cmd sent always fails after a write, probs due to broken write read_write_status check
            fifo::seek(Some(cyl))?
        };
