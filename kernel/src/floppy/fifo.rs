use super::{
    DRIVE_ONE, FloppyError, FloppyPort, RETRIES, ST0_ERR_OR_RESET, TIMEOUT, disk, init_fdc, motor,
};
...
pub unsafe fn send_byte(byte: u8) -> Result<(), FloppyError> {
    ...
    motor::disable_motor();
    Err(FloppyError::FifoTimeout(FifoError::Write(byte)))
}
...
pub unsafe fn read_byte() -> Result<u8, FloppyError> {
    ...
    motor::disable_motor();
    Err(FloppyError::FifoTimeout(FifoError::Read))
}

/// Sends command `cmd` to the FIFO port with `params` as it's parameters.
///
/// This function will reset the controller if an error occurred.
/// # Safety
...
/// # Safety
/// The caller must ensure that this function is only called ONCE, immediately after a Seek or,
/// Recalibrate and FOUR TIMES after a Reset command, the correct state must be passed and
/// a disk operation must not be in progress if the state is `SenseIntState::SeekOrRecal`.
pub unsafe fn sense_interrupt(state: SenseIntState) -> Result<(), FloppyError> {
    ...
    let reset_passed = st0 == ST0_ERR_OR_RESET | drive_num; // if st= 0xC0 | drive num, the reset completed
