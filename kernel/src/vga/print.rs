...
/*!
    kernel/src/vga/print.rs

    Defines print macros for writing to the vga text buffer.
...
use super::buffers::{BUFFER_HEIGHT, BUFFER_WIDTH, YoinkedBuffer};
use super::cursor::{self, CursorPos, CursorShift};
...
/// A character value supported by the VGA's text mode.
...
impl VGAChar {
      ...
      /// Constructs a new color using `fg` as the text color
      /// and `bg` as the background color.
...
#[macro_export]
macro_rules! print {
    (fg = $fg:ident, bg = $bg:ident, $($args:tt)+) => (
        $crate::vga::print::_print(
            format_args!($($args)+),
            $crate::vga::print::Color::$fg,
            $crate::vga::print::Color::$bg
        ));

    (fg = $fg:ident, $($args:tt)+) => (
        $crate::print!(fg = $fg, bg = Black, "{}", format_args!($($args)+)));

    (bg = $fg:ident, $($args:tt)+) => (
        $crate::print!(fg = White, bg = $fg, "{}", format_args!($($args)+)));

    ($($args:tt)+) => (
        $crate::print!(fg = White, bg = Black, "{}", format_args!($($args)+)));
}
...
macro_rules! println {
    ...
    (fg = $fg:ident, $($arg:tt)+) => (
        $crate::print!(fg = $fg, bg = Black, "{}\n", format_args!($($arg)+)));

    (bg = $bg:expr, $($arg:tt)+) => (
        $crate::print!(fg = White, bg = $bg, "{}\n", format_args!($($arg)+)));
...
/// Writes `byte` to VGA as a character using `fg` as the text color,
/// and `bg` as the background color.
pub fn write_char(byte: u8, fg: Color, bg: Color) {
      ...
      if let Some(mut buf) = YoinkedBuffer::try_yoink() {
            buf.buffer()[row as usize][col as usize] =
                  VGAChar::new(byte, fg, bg);
      }
...
fn newline() {
      if let Some(mut buf) = YoinkedBuffer::try_yoink() {
            ...
            // If we've reached the end, move all rows (except topbar) up one
            ...
            if row >= BUFFER_HEIGHT - 1 {
                  ...
                  for col in 0..BUFFER_WIDTH {
                        buf[BUFFER_HEIGHT as usize - 1][col as usize] =
                              VGAChar::SPACE
                  }
            } else {
...
pub fn delete_prev_char() {
      if let Some(mut buf) = YoinkedBuffer::try_yoink() {
            ...
            if col == 0 {
                  buf.buffer()[row as usize - 1][BUFFER_WIDTH as usize - 1] =
                        VGAChar::SPACE;
                  drop(buf);
