...
macro_rules! restore_state {
      () => {
            ...
            pop rax
            pop rdi
            lock dec dword ptr int_handler_count" // (maybe) re-allow CA use
      };
}
    ...
    ($errcode: expr, $inc: expr) => {{
        ...
        extern "C" fn wrapper() -> ! {
            naked_asm!(
                ...
                concat!("mov word ptr [cont_errcode], ", stringify!($errcode)),
                "mov rdi, rsp", // store stack frame in first arg
                "add rdi, 9*8", // offset the 9 registers that just got pushed
                "call cont",
                restore_state!(),
                concat!("add qword ptr [rsp], ", $inc), // inc return address
                "iretq"
            )
        }
...
extern "sysv64" fn cont(frame: IntStackFrame) {
      #[unsafe(export_name = "cont_errcode")]
...
impl Idt {
      /// Creates an initialised table, with all it's required entries set.
      ...
      fn set_handler(
            &mut self, entry_id: usize, ist: Option<u8>, handler: Handler,
      ) {
            self.0[entry_id] =
                  InterruptDescriptor::new(handler, ist.unwrap_or_default())
      }
...
impl InterruptDescriptor {
      /// Returns a new descriptor using `handler` as
      /// it's offset and `ist` for the IST.
      #[rustfmt::skip]
      fn new(offset_ptr: Handler, ist: u8) -> Self {
            /// Present, dpl = 0, gate type = interrupt
            const FLAGS: u8 = 0b1_00_0_1111;

            if ist > 0b111 { // other bits are reserved
                  warn!("attempted creating an int descriptor with an \
                        ist > 7 ({ist}), which will be truncated!");
            }
            let ist = ist & 0b111;
...
/// Since IRQs 7 & 15 aren't used by sunflower though, it's not that bad.
#[inline(never)]
...
extern "C" fn double_fault_handler() -> ! {
      naked_asm!(
            "cli",
            "pop rax", // remove empty error code double faults push
            "mov rdi, rsp",                // stack frame as first param
            "call print_df_info",          // print error info
            "mov rax, 0xDFDFDFDFDFDFDFDF", // pseudo error message which
                                           // can be viewed in QEMU
...
extern "C" fn print_df_info(frame: IntStackFrame) {
      // Safety: Whoever was holding that buffer is not going to be returned to
      // anytime soon
      ...
      println!(
        ...
        it:\n{frame}\nSince double faults are pretty nasty, sunflower can't\
        trust any kernel services to get keyboard input or wait, so you'll \
        have to restart your device manually"
      );
}
...
extern "C" fn timer_handler() -> ! {
      naked_asm!(
            savestate!(), // save for decrease_motor_time & eoi
            "lock inc qword ptr [TIME]",
...
extern "C" fn rtc_handler() -> ! {
      ...
      naked_asm!(
            ...
            "push ax",
            savestate!(),
            "cmp byte ptr [RTC_UPDATE_ENDED], 1", // has the update ended?
            "je rtc_ret",                         // if yes we're already done!

            "mov dx, 0x70", // dx = CMOS index register
            "mov al, 0x8C", // al = select register C
            "out dx, al",   // register C selected!
            "mov dx, 0x71", // dx = CMOS data register
            "in al, dx",    // read selected register please!
            "mov ah, al",
            "or ah, 16",    // set bit 4
            "cmp al, ah",   // is bit 4 of reg C set?
            "jne rtc_ret",  // if not we'll have to try again later...
            "mov byte ptr [RTC_UPDATE_ENDED], 1",
