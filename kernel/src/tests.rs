...
pub fn exit_qemu(error: bool) -> ! {
      ...
      const SUCCESS_EXIT_CODE: u8 = 0x42;

      // Safety: Port 0xF4 can be used when ran in tests to exit QEMU
      let exitcode = SUCCESS_EXIT_CODE + error as u8;
      unsafe { ports::writeb_nodummy(Port::QemuExit, exitcode) }
...
pub fn run_tests(tests: &[&dyn Test]) -> ! {
      ...
      // from being run, PANIC! exits QEMU when hitting page faults in test
      // builds
      loop {
            unsafe { asm!("push rax") }
      }
